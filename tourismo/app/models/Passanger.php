...
class Passanger extends Eloquent {
	...
	public static function psgForPeriod($cat, $dst, $from, $to)
	{
		...
		$psgsel = DB::table('passanger')
	            ...
	            ->select('passanger.*', 'reservations.id AS res_id', 'reservations.reservation_number', 'reservations.travel_date', 'destinations.country', 'destinations.town', 'accomodations.type', 'accomodations.name AS acc_name')
	...
	public static function psgGreenList($cat, $dst, $from, $to)
	{
		$passangers = null;
		$psgsel = DB::table('passanger')
	            ->join('passangers', 'passangers.passanger_id', '=', 'passanger.id')
	            ...
	            ->join('accomodations', 'accomodations.id', '=', 'accomodation_units.accommodations_id')
	            ->join('categories', 'categories.id', '=', 'travel_deals.category_id')
	            ->select('passanger.*', 'reservations.reservation_number', 'reservations.note', 'reservations.travel_date', 'destinations.country', 'destinations.town', 'accomodations.type', 'accomodations.name AS acc_name', 'travel_deals.transportation')
	            ->where('reservations.status', '!=', 'Storno');
		if ($cat != null && $cat != "")
			$psgsel = $psgsel->where('categories.name', 'LIKE', $cat);
		if ($dst != null && $dst != "") {
	...
	public static function promoters($cat, $dst, $from, $to)
	{
		$passangers = null;
		$psgsel = DB::table('passanger')
	            ->join('passangers', 'passangers.passanger_id', '=', 'passanger.id')
	            ...
	            ->join('accomodations', 'accomodations.id', '=', 'accomodation_units.accommodations_id')
	            ->join('categories', 'categories.id', '=', 'travel_deals.category_id')
	            ->select('passanger.*', 'reservations.id AS res_id', 'reservations.reservation_number', 'reservations.discount', 'reservations.travel_date', 'destinations.country', 'destinations.town', 'accomodations.type', 'accomodations.name AS acc_name', 'travel_deals.transportation')
	            ->where('reservations.status', '!=', 'Storno');
		if ($cat != null && $cat != "")
			$psgsel = $psgsel->where('categories.name', 'LIKE', $cat);
		if ($dst != null && $dst != "") {
